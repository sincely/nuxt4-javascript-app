...
const { instance: batteryInstance } = useTranslateX({
  el: batteryModuleRef,
  from: -600,
  to: 0,
  delay: 0,
...
const { instance: motorInstance } = useTranslateY({ el: motorModuleRef, from: -600, to: 0, delay: 0, duration: 600 })
const { instance: parkInstance } = useTranslateX({ el: parkModuleRef, from: 600, to: 0, delay: 0, duration: 600 })
...
<template>
  ...
  >
    ...
    <div class="grid grid-cols-25 h-full row-span-12 mt-6 justify-around md:(row-span-6 row-start-4 mt-0)">
      <!-- 模块 -->
      <div
        ref="batteryModuleRef"
        class="card md:!col-start-2"
      >
